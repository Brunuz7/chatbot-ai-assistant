...
export class UserSettingController {
  ...
  static async updateTagging(req: AuthRequest, res: Response) {
    try {
  ...
  static async updateSchedule(req: AuthRequest, res: Response) {
    try {
      const row = await UserSettingService.updateSchedule(req.user!.sub, req.body ?? {});
      res.json(row);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Erro desconhecido';
      if (message.includes('inválido')) return res.status(400).json({ error: message });

      console.error('UserSetting schedule:', err);
      res.status(500).json({ error: 'Falha ao actualizar horário de resposta' });
    }
  }
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error: 'Envie uma amostra de voz antes de ativar o modo clonado.',
        });
      }
      ...
      const message = err instanceof Error ? err.message : 'Erro desconhecido';
      if (message.includes('inválido') || message.includes('vazio')) return res.status(400).json({ error: message });
