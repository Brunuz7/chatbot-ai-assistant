...
export class BulkMessageCampaignController {
  ...
  static async getById(req: AuthRequest, res: Response) {
    try {
      ...
      if ((err as Error).message === 'not_found') return res.status(404).json({ error: 'Campanha não encontrada' });
  ...
  static async cancel(req: AuthRequest, res: Response) {
    try {
      ...
      res.json({ ...row, tag_ids: parseTagIds(row.tag_ids) });
    } catch (err: unknown) {
      if ((err as Error).message === 'not_found') return res.status(404).json({ error: 'Campanha não encontrada' });
