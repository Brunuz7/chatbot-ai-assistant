...
const PERIODS = new Set<DashboardStatsPeriod>(['day', 'week', 'month']);

function parsePeriod(raw: unknown): DashboardStatsPeriod {
  const p = String(raw ?? 'day').toLowerCase();
  return PERIODS.has(p as DashboardStatsPeriod) ? (p as DashboardStatsPeriod) : 'day';
}
...
export class ConversationController {
  ...
  static async getMessageStats(req: AuthRequest, res: Response) {
    try {
      ...
      const overview = await DashboardService.getOverview(req.user!.sub, period);
      res.json(overview);
    } catch (error) {
