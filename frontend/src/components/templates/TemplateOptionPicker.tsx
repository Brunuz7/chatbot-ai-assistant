import React from 'react';
import { Check } from 'lucide-react';
import { getTemplateAccentStyles } from './templateWizardConstants';

type TemplateOption = {
  id: string;
  title: string;
  description: string;
  icon: React.ComponentType<{ size?: number; className?: string; strokeWidth?: number }>;
  accent: string;
};

type TemplateOptionPickerProps<T extends string> = {
  value: T;
  onChange: (id: T) => void;
  options: TemplateOption[];
  ariaLabel: string;
  columns?: '2' | '3' | '4' | '5';
  disabled?: boolean;
};

const colClass: Record<string, string> = {
  '2': 'sm:grid-cols-2',
  '3': 'sm:grid-cols-2 xl:grid-cols-3',
  '4': 'sm:grid-cols-2 xl:grid-cols-4',
  '5': 'sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5',
};

export function TemplateOptionPicker<T extends string>({
  value,
  onChange,
  options,
  ariaLabel,
  columns = '3',
  disabled = false,
}: TemplateOptionPickerProps<T>) {
  return (
    <div
      className={`grid grid-cols-1 gap-2 ${colClass[columns]}`}
      role="radiogroup"
      aria-label={ariaLabel}>
      {options.map((option) => {
        const selected = value === option.id;
        const styles = getTemplateAccentStyles(option.accent);
        const Icon = option.icon;

        return (
          <button
            key={option.id}
            type="button"
            disabled={disabled}
            onClick={() => onChange(option.id as T)}
            className={[
              'group relative flex min-h-[5.5rem] flex-col gap-2.5 rounded-xl border p-3 text-left transition-all duration-200',
              'hover:-translate-y-px hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-a40',
              'disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-none',
              selected
                ? `${styles.selected} ring-2 ${styles.ring} shadow-md`
                : 'border-slate-200/90 bg-white hover:border-slate-300 dark:border-slate-700/90 dark:bg-slate-900/60 dark:hover:border-slate-600',
            ].join(' ')}
            aria-pressed={selected}>
            {selected ? (
              <span className="absolute right-2 top-2 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-white shadow-sm">
                <Check size={12} strokeWidth={3} aria-hidden />
              </span>
            ) : null}
            <span className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${styles.icon}`}>
              <Icon size={18} strokeWidth={2} aria-hidden />
            </span>
            <span className="min-w-0 flex-1 pr-4">
              <span className="block text-base font-semibold leading-tight text-slate-900 dark:text-white">
                {option.title}
              </span>
              <span className="mt-1 block text-sm leading-snug text-slate-500 dark:text-slate-400">
                {option.description}
              </span>
            </span>
          </button>
        );
      })}
    </div>
  );
}
